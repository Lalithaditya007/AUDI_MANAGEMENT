    ...
    <div className="min-h-screen flex flex-col">
          ...
          >
            ...
            <div className="flex flex-wrap gap-4 justify-center mt-8">
              <motion.a
                ...
                whileTap={{ scale: 0.95 }}
                href="/auditoriums" // Link remains the same
                className="px-8 py-3 bg-red-600 text-white rounded-full font-semibold hover:bg-red-700 transition-colors duration-300"
              >
                Explore Auditoriums
              </motion.a>

              {/* --- MODIFIED: Conditional "Book Now" Button --- */}
              {/* Only show if NOT logged in as admin */}
              {!(isLoggedIn && userRole === 'admin') && (
                  <motion.a
                    whileHover={{ scale: 1.05 }}
                    whileTap={{ scale: 0.95 }}
                    // Set href based on login status
                  ...
                  // Alternatively, using Link component:
                  // <motion.custom component={Link} ... to={isLoggedIn ? "/book-auditorium" : "/login"}> Book Now </motion.custom>
              )}
              {/* --- END MODIFICATION --- */}
      ...
      <section className="py-20 relative overflow-hidden">
        ...
        <motion.div
          ...
          transition={{ duration: 0.8 }}
          viewport={{ once: true }}
          className="w-[90%] mx-auto px-4 relative z-10"
        >
          <div className="text-center mb-12">
             <motion.h2 initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.2 }} className="text-4xl md:text-5xl font-bold mb-4"> <span className="text-[#82181A]"> Current & Upcoming Events </span> </motion.h2>
             <motion.div initial={{ width: 0 }} whileInView={{ width: "100px" }} transition={{ duration: 0.8, delay: 0.4 }} className="h-1 bg-red-400 mx-auto rounded-full" />
          </div>
          <div className="w-full mx-auto backdrop-blur-md bg-white/10 rounded-2xl p-6 shadow-2xl border-2 border-[#82181A]/20">
             <Events />
          </div>
          <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} transition={{ duration: 0.6, delay: 0.6 }} className="mt-8 text-center">
          </motion.div>
        </motion.div>
      </section>
      ...
      <section className="py-20 bg-gradient-to-br from-red-50 via-white to-red-50">
        <div className="container mx-auto px-4">
          ...
          <div className="flex flex-wrap justify-center">
            <motion.div className="w-full md:w-10/12" initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} transition={{ duration: 0.8 }} viewport={{ once: true }}>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8 relative">
                ...
                <motion.div className="bg-white p-6 rounded-xl shadow-lg border border-gray-100 text-center relative z-10" whileHover={{ y: -5 }} transition={{ type: "spring", stiffness: 300 }}> <div className="w-16 h-16 rounded-full bg-red-100 text-red-600 flex items-center justify-center mx-auto mb-4 text-2xl font-bold">1</div> <h3 className="text-xl font-semibold mb-3">Sign In</h3> <p className="text-gray-500">Use your institutional credentials to access our booking platform</p> </motion.div>
                <motion.div className="bg-white p-6 rounded-xl shadow-lg border border-gray-100 text-center relative z-10" whileHover={{ y: -5 }} transition={{ type: "spring", stiffness: 300 }}> <div className="w-16 h-16 rounded-full bg-red-100 text-red-600 flex items-center justify-center mx-auto mb-4 text-2xl font-bold">2</div> <h3 className="text-xl font-semibold mb-3">Select & Request</h3> <p className="text-gray-500">Choose your preferred auditorium, date and time, and submit your event details</p> </motion.div>
                <motion.div className="bg-white p-6 rounded-xl shadow-lg border border-gray-100 text-center relative z-10" whileHover={{ y: -5 }} transition={{ type: "spring", stiffness: 300 }}> <div className="w-16 h-16 rounded-full bg-red-100 text-red-600 flex items-center justify-center mx-auto mb-4 text-2xl font-bold">3</div> <h3 className="text-xl font-semibold mb-3">Confirmation</h3> <p className="text-gray-500">Receive approval notification and prepare for your successful event</p> </motion.div>
              </div>
            </motion.div>
          </div>
          <motion.div className="text-center mt-12" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.4 }} viewport={{ once: true }}>
            <a href="/login" className="inline-flex items-center px-6 py-3 bg-red-600 text-white font-medium rounded-lg hover:bg-red-700 transition-colors"> Get Started <svg className="w-5 h-5 ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 7l5 5m0 0l-5 5m5-5H6"></path></svg> </a>
          </motion.div>
        </div>
      </section>
      ...
      <section className="py-20 bg-gradient-to-br from-red-700 to-red-900 relative overflow-hidden">
        <div className="absolute top-0 left-0 w-full h-40 bg-[url('/src/assets/pattern.png')] opacity-5"></div>
        <div className="absolute -bottom-20 -right-20 w-80 h-80 bg-red-800 rounded-full opacity-30"></div>
        <div className="absolute -bottom-10 -left-10 w-40 h-40 bg-red-800 rounded-full opacity-30"></div>
        <div className="container mx-auto px-4 relative z-10">
          <div className="max-w-4xl mx-auto text-center">
            <motion.h2 initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} viewport={{ once: true }} className="text-4xl md:text-5xl font-bold mb-6 text-white"> Ready to Host Your Next Event? </motion.h2>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} transition={{ duration: 0.6, delay: 0.4 }} viewport={{ once: true }} className="flex flex-wrap gap-6 justify-center">
              {/* --- MODIFIED: Conditional "Book Now" Button Logic in Final CTA --- */}
              {!(isLoggedIn && userRole === 'admin') && (
                <motion.a
                  whileHover={{ scale: 1.05 }}
                  whileTap={{ scale: 0.95 }}
                  href={isLoggedIn ? "/book-auditorium" : "/login"} // Link conditionally
                  className="px-8 py-4 bg-white text-red-800 rounded-lg font-bold text-lg shadow-lg hover:shadow-xl transition-all duration-300"
                >
                  Book an Auditorium
                </motion.a>
              )}
              {/* --- END MODIFICATION --- */}
              <motion.a whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} href="/auditoriums" className="px-8 py-4 bg-transparent border-2 border-white text-white rounded-lg font-bold text-lg shadow-lg hover:bg-white/10 transition-all duration-300"> Explore Options </motion.a>
            </motion.div>
          </div>
        </div>
      </section>
